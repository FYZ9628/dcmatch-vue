<template>
  <div style="height: 550px; background-color: #ffffff; margin-bottom: 10px; text-align: left">
    ...
    <el-table
      :data="adminList.filter(data => !search || data.phone.toLowerCase().includes(search.toLowerCase())
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          <el-button
            @click.native.prevent="editUser(scope.$index, adminList)"
            type="primary"
            size="mini">
            编辑
          </el-button>
          <el-button
            @click.native.prevent="deleteUser(scope.$index, adminList)"
            type="danger"
            size="mini">
            删除
          </el-button>
